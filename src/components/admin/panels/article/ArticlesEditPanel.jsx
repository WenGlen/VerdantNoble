...
export default function ArticlesEditPanel({
  ...
  async function handleImageUpload(file, fieldKey) {
    ...
    if (!["image/jpeg", "image/jpg", "image/png"].includes(file.type)) {
      setUploadError((prev) => ({
        ...prev,
        [fieldKey]: "圖片格式錯誤，僅支援 jpg、jpeg 與 png",
      }));
      return;
    }
    ...
    try {
      ...
      if (res.data?.imageUrl) {
        if (fieldKey === "image") {
          eventHandlerEditingArticle({
            target: { name: "image", value: res.data.imageUrl },
          });
        } else {
  ...
  return (
    <div className="h-screen w-[600px] xl:w-[400px] flex flex-col gap-3 overflow-hidden p-4 bg-admin-card-edit tablet:rounded-md">
      ...
      <div className="shrink-0 flex flex-col gap-2">
        <div className="flex items-center justify-between gap-2">
          <div className="flex gap-1 shrink-0">
            <button
              ...
              onClick={() =>
                setEditingArticle((prev) => ({ ...prev, isPublic: true }))
              }
            >
              公開
            </button>
            <button
              ...
              className={`btn-select px-2 py-1 whitespace-nowrap ${!editingArticle.isPublic ? "active" : ""}`}
              onClick={() =>
                setEditingArticle((prev) => ({ ...prev, isPublic: false }))
              }
            >
              隱藏
            </button>
          ...
          <div className="flex items-center gap-1 flex-1 min-w-0">
            <label htmlFor="author" className="text-xs min-w-8 shrink-0">
              作者
            </label>
            <input
              id="author"
        ...
        <div className="flex items-center gap-1">
          <label htmlFor="description" className="text-xs min-w-8">
            摘要
          </label>
          <input
            id="description"
      ...
      <div className="flex-1 min-h-0 flex flex-col">
        ...
        <div className="flex gap-1 shrink-0">
          {[
            ...
            {
              key: "related",
              label: `相關商品（${selectedProductIds.length}/3）`,
            },
          ].map((tab) => (
            <button
              key={tab.key}
                ...
                {editingArticle.tag.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-1">
                    {editingArticle.tag.map((t, i) => (
                      <span
                        key={i}
                        className="flex items-center gap-0.5 text-xs px-2 py-0.5 rounded bg-slate-200 text-slate-700"
                      >
                        {t}
                        <button
                          type="button"
                ...
                {blocks.length === 0 ? (
                  ...
                  <div className="flex flex-col gap-2">
                    {blocks.map((block, index) => (
                      <div
                        key={index}
                        className="bg-white border border-slate-200 rounded p-2 flex flex-col gap-1.5"
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex gap-1">
                            <button
                              type="button"
                              className="text-xs px-1.5 py-0.5 rounded bg-slate-100 hover:bg-slate-200 border-none cursor-pointer disabled:opacity-30"
                            ...
                            <button
                              type="button"
                              className="text-xs px-1.5 py-0.5 rounded bg-red-50 text-red-500 hover:bg-red-100 border-none cursor-pointer"
                        ...
                        {block.type === "heading" && (
                          <input
                            ...
                            className="admin-edit-panel__input text-base font-semibold"
                          />
                        )}
                        ...
                        {(block.type === "text" ||
                          ...
                          <textarea
                            ...
                            rows={3}
                            className={`admin-edit-panel__input resize-y text-sm ${block.type === "emphasis" ? "italic" : ""}`}
                          />
                        )}
                          ...
                          <div className="flex flex-col gap-1">
                            <div className="flex gap-1">
                              <input
                                ...
                                className="admin-edit-panel__input text-sm"
                              />
                              <input
                                type="file"
                                accept="image/jpeg,image/jpg,image/png"
                                ref={(el) =>
                                  (fileInputRefs.current[`block-${index}`] = el)
                                }
                                onChange={(e) =>
                                  handleFileSelect(e, `block-${index}`)
                                }
                                className="hidden"
                              />
                              <button
                ...
                <div className="flex flex-col gap-1">
                  ...
                  {selectedProductIds.map((id) => {
                    ...
                    return (
                      <div
                        key={id}
                        className="flex items-center gap-2 px-2 py-1.5 rounded bg-admin-card text-xs"
                      >
                        {p.imageUrl && (
                          <img
                            src={p.imageUrl}
              ...
              {selectedProductIds.length < 3 && (
                <div className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    className="admin-edit-panel__input text-xs"
                  />
                  <div className="flex flex-col rounded bg-admin-card-50 overflow-hidden">
                    {filteredProducts.length === 0 ? (
                      <p className="text-xs text-admin-text-muted text-center py-3">
                        無符合商品
                      </p>
                    ) : (
                      filteredProducts.map((p) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {isSelected ? (
                              <span className="text-admin-text-muted shrink-0">
                                已選
                              </span>
                            ) : (
                              <span className="text-admin-primary shrink-0">
                                ＋
                              </span>
                            )}
                          </button>
