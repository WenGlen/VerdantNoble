export default function ArticlesFocusPanel({ focus, setFocus, editArticle, setMod }) {
    ...
    const blockTypeLabel = { heading: '🔤', text: '📄', emphasis: '💬', image: '🖼' };
    ...
    function renderBlockPreview(blocks) {
        ...
        return blocks.map((block, i) => {
            ...
            if (block.type === 'emphasis') {
                return (
                    <p key={i} className="text-xs italic text-admin-primary whitespace-pre-wrap">
                ...
                return (
                    <div key={i} className="flex flex-col gap-1">
                        <p className="text-xs text-admin-text-muted">
                            {blockTypeLabel.image} 圖片{block.caption ? `：${block.caption}` : ""}
                        </p>
                        {block.url && (
                ...
                <div className="h-fit xl:h-screen w-[600px] xl:w-[400px] flex-col-between gap-2 p-4 overflow-x-auto bg-admin-card-focus tablet:rounded-md">
                    ...
                    <div className="w-full flex flex-col gap-3 py-2 shrink-0">
                        ...
                        {focus.image && (
                            <div className="w-full h-[120px] bg-slate-200 overflow-hidden rounded">
                                <img src={focus.image} alt={focus.title} className="w-full h-full object-cover" />
                            </div>
                        )}

                        {/* 狀態列 */}
                        <div className="flex items-center gap-2 text-xs">
                            <span className={`px-2 py-0.5 rounded font-semibold ${focus.isPublic ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}>
                                {focus.isPublic ? "公開" : "未公開"}
                            </span>
                            <span className="text-admin-text-muted">{formatTimestamp(focus.create_at)}</span>
                            <span className="text-admin-text-muted">作者：{focus.author}</span>
                        </div>

                        {/* 標題 & 摘要 */}
                        <div className="space-y-1">
                            <h3 className="text-admin-text text-base font-semibold">{focus.title}</h3>
                            <p className="text-xs text-admin-text-muted">{focus.description}</p>
                        </div>
                        ...
                        {Array.isArray(focus.tag) && focus.tag.length > 0 && (
                            <div className="flex flex-wrap gap-1">
                                {focus.tag.map((t, i) => (
                                    <span key={i} className="text-xs px-1.5 py-0.5 rounded bg-admin-card-50 text-admin-text">{t}</span>
                                ))}
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex justify-between pt-2 shrink-0">
                        <button
                            ...
                            onClick={() => { setMod("update"); editArticle(focus); }}
                        >
                            編輯
                        </button>
                        <button
