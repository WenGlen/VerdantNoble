          ...
          <div className="shrink-0 flex flex-col gap-2 py-2">
            ...
            <div className="flex-row-center-center gap-8 text-sm">
              <div className="w-[160px] space-y-1">
                <FocusPanelItem
                  label="是否啟用："
                  value={focus.is_enabled ? "上架" : "隱藏"}
                />
                <FocusPanelItem label="原價：" value={focus.origin_price} />
                <FocusPanelItem
                  label="折扣："
                  value={`${Math.round((focus.price / focus.origin_price) * 100)}%`}
                />
              ...
              <div className="w-[160px] space-y-1">
                <FocusPanelItem
                  label="更新："
                  value={formatDate(focus.updated_at)}
                />
                <FocusPanelItem label="分類：" value={focus.category} />
                <FocusPanelItem
                  label="已售數量："
                  value={focus.soldQuantity || 0}
                />
              ...
              {activeTab === "content" && (
                <div className="flex flex-col gap-4 text-sm">
                  ...
                  {!focus.content && !focus.description && !focus.story && (
                    <p className="text-xs text-admin-text-muted text-center py-6">
                      無內文資料
                    </p>
                  )}
                </div>
                    ...
                    care.map((tip, i) => (
                      <div key={i} className="flex items-start gap-2">
                        ...
                        <div className="flex flex-col gap-0.5 flex-1 min-w-0">
                          ...
                          {tip.description && (
                            <p className="text-admin-text whitespace-pre-wrap">
                              {tip.description}
                            </p>
                          )}
                        </div>
