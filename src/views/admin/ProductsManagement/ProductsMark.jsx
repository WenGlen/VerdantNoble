  ...
  async function saveChanges() {
    ...
    try {
      ...
      for (const product of toUpdate) {
        ...
        const imagesUrl = (product.imagesUrl || []).filter(
          (u) => u?.trim() !== "",
        );
        await axios.put(
          `${VITE_API_URL}/api/${VITE_API_PATH}/admin/product/${product.id}`,
          {
            data: {
              title: product.title,
      ...
      dispatch(
        createAsyncDashboardToast({ message: "精選排序已儲存", success: true }),
      );
    } catch {
      dispatch(
        createAsyncDashboardToast({
  ...
  return (
    <div className="h-screen flex flex-col px-8 pt-8 pb-4 max-w-4xl mx-auto overflow-hidden">
      ...
      <div className="w-full flex-row-between-center gap-2">
        <h3>
          首頁顯示順序（{featuredOrder.length} / {MAX_FEATURED}）
        </h3>
        ...
        <div className="flex flex-col gap-1.5">
          {Array.from({ length: MAX_FEATURED }, (_, i) => {
            ...
            return (
              ...
              >
                <span className={`font-bold w-4 shrink-0`}>{i + 1}</span>
                {product ? (
                  <>
                    ...
                    <span className="flex-1 truncate font-semibold text-admin-text">
                      {product.title}
                    </span>
                    <span className="text-admin-text-muted text-xs shrink-0">
                      NT${product.price}
                    </span>
      ...
      <div className="w-full flex-row-start-center gap-2">
        <h3>所有商品</h3>{" "}
        <p className="text-sm text-admin-text-muted">（勾選加入精選）</p>
      </div>
      <div className="flex-1 overflow-y-auto min-h-0">
        <div className="flex flex-col gap-1">
          {products
            .filter((p) => p.category === "精品")
            .map((product) => {
              const isChecked = featuredOrder.includes(product.id);
              const isAtMax =
                !isChecked && featuredOrder.length >= MAX_FEATURED;
