...
export default function ArticlesManagement() {
  ...
  async function viewArticle(item) {
    ...
    try {
      const res = await axios.get(
        `${VITE_API_URL}/api/${VITE_API_PATH}/admin/article/${item.id}`,
      );
  ...
  async function deleteArticle(id) {
    ...
    try {
      await axios.delete(
        `${VITE_API_URL}/api/${VITE_API_PATH}/admin/article/${id}`,
      );
      getArticles();
      setFocus({});
      setMod("view");
      setDeleteTargetId("");
      dispatch(
        createAsyncDashboardToast({ message: "成功刪除文章", success: true }),
      );
    } catch (error) {
      setMod("view");
      dispatch(
        createAsyncDashboardToast(
          error.response?.data || { message: "刪除文章失敗", success: false },
        ),
      );
    } finally {
      setDeleting(false);
  ...
  async function uploadArticle(currentMod) {
    ...
    try {
      if (currentMod === "add") {
        await axios.post(
          `${VITE_API_URL}/api/${VITE_API_PATH}/admin/article`,
          uploadItem,
        );
        dispatch(
          createAsyncDashboardToast({ message: "新增文章成功", success: true }),
        );
      } else {
        await axios.put(
          `${VITE_API_URL}/api/${VITE_API_PATH}/admin/article/${focus.id}`,
          uploadItem,
        );
        dispatch(
          createAsyncDashboardToast({ message: "更新文章成功", success: true }),
        );
      }
      getArticles();
    } catch (error) {
      dispatch(
        createAsyncDashboardToast(
          error.response?.data || { message: "上傳文章失敗", success: false },
        ),
      );
    } finally {
      setUploading(false);
