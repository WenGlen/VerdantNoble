...
export default function ArticlesMark() {
    ...
    try {
      ...
      while (hasMore) {
        const res = await axios.get(
          `${VITE_API_URL}/api/${VITE_API_PATH}/admin/articles?page=${page}`,
        );
        if (res.data.success) {
  ...
  const featuredArticle = articles.find(
    (a) => Array.isArray(a.tag) && a.tag.includes(FEATURED_TAG),
  );
  ...
  async function setFeatured(article) {
    ...
    try {
      ...
      const newRes = await axios.get(
        `${VITE_API_URL}/api/${VITE_API_PATH}/admin/article/${article.id}`,
      );
      ...
      if (featuredArticle && featuredArticle.id !== article.id) {
        const oldRes = await axios.get(
          `${VITE_API_URL}/api/${VITE_API_PATH}/admin/article/${featuredArticle.id}`,
        );
        const oldArticle = oldRes.data.article;
        const oldTags = (oldArticle.tag || []).filter(
          (t) => t !== FEATURED_TAG,
        );
        await axios.put(
          `${VITE_API_URL}/api/${VITE_API_PATH}/admin/article/${featuredArticle.id}`,
      ...
      setArticles((prev) =>
        prev.map((a) => {
          ...
          if (a.id === featuredArticle?.id)
            return {
              ...a,
              tag: (a.tag || []).filter((t) => t !== FEATURED_TAG),
            };
          return a;
        }),
      );
  ...
  async function clearFeatured() {
    ...
    try {
      ...
      dispatch(
        createAsyncDashboardToast({ message: "已取消精選文章", success: true }),
      );
    } catch {
      dispatch(
        createAsyncDashboardToast({
    ...
    <div className="p-8 max-w-3xl mx-auto">
              ...
              {featuredArticle.image && (
                <div className="w-14 h-14 shrink-0 rounded overflow-hidden bg-admin-card-50">
                  <img
                    ...
                    alt={featuredArticle.title || "精選文章封面"}
                    className="w-full h-full object-cover"
                  />
                </div>
              )}
              <div className="min-w-0">
                <p className="font-semibold text-sm text-admin-text truncate">
                  {featuredArticle.title}
                </p>
                <p className="text-xs text-admin-text-muted">
                  {featuredArticle.author}{" "}
                  {formatTimestamp(featuredArticle.create_at)}
                </p>
              </div>
            </div>
            <button
      ...
      <div className="flex flex-col gap-2">
        {articles.map((article) => {
          const isFeatured =
            Array.isArray(article.tag) && article.tag.includes(FEATURED_TAG);
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {!article.isPublic && (
                    <span className="text-xs px-1.5 py-0.5 rounded bg-admin-card-50 text-admin-text-muted shrink-0">
                      未公開
                    </span>
                  )}
                </div>
