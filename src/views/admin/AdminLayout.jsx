                ...
                <div className="h-full flex-col-between-start gap-6 py-16">
                    ...
                    {adminPagesItems.map((item) => (
                        <div key={item.path} className="">
                            <NavLink key={item.path} to={item.path} className="flex-row-center-center gap-2 no-underline" >
                                    <img src={item.icon} alt={item.label} className={`w-6 h-6 ${item.path === location.pathname ? ' opacity-100' : 'opacity-50'}`} />
                                    <div className={`admin-btn-nav whitespace-nowrap opacity-0 group-hover:opacity-100 transition-all delay-100 duration-300 ${item.path === location.pathname ? ' active' : ''}`}>
                                        {item.label}
                                    </div>
                            </NavLink>
                        </div>
